...
## 通常シフトの形（全員一律の前提条件・2026-06-30 更新）

`_apply_shift_shape` が全員に課す「普通のフルシフト1本」の形。現実のシフト実態に合わせ、
**1日に2回に分けて入る細切れを廃止**した（`src/solver/engine.py`）。

- **連続1ライン（Hard）**: 1日の勤務ブロック数 ≤ `SHIFT_MAX_BLOCKS_PER_DAY=1`。立ち上がり start[k] の数で数える。
- **基本最大8時間（Hard）**: 1日の在席コマ合計 ≤ `SHIFT_MAX_HOURS_PER_DAY=8`（コマ数換算）。営業時間が長くても1人は最大8時間。
- **最低3時間（Soft）**: 出勤する日は合計 ≥ `SHIFT_MIN_BLOCK_SLOTS`（3時間）。不足(5000)より小さい減点なので、解けない時だけ破る。
- **1日1ポジション（Hard）**: 30分ごとのポジション交代を防ぐ。

> デモデータ（`scripts/gen_demo_data.py`）はこの前提に合わせ、昼帯と夜帯を別人で埋める1ライン編成で
> 充足するよう人数配分し、店ごとに営業時間をばらけさせた（`scripts/verify_demo_data.py` で実測検証）。
